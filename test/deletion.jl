...
@testset "Scalar Parameters" begin
    ...
    @variable(m, x)
    var = build_variable(error, inf4, [0.5, NaN, NaN], check = false)
...
@testset "JuMP.delete (SemiInfinite Variables)" begin
    ...
    @variable(m, x)
    var = build_variable(error, inf, [NaN, 0.5], check = false)
    rv = add_variable(m, var)
    var = build_variable(error, inf, [NaN, 0.0], check = false)
...
@testset "JuMP.delete (Infinite Variables)" begin
    ...
    var = build_variable(error, x, [0.5], check = false)
    rv = add_variable(m, var)
    data = TestData(par, 0, 1)
    meas1 = measure(x + y + par, data)
    meas2 = measure(y, data)
...
@testset "JuMP.delete (Derivatives)" begin 
    ...
    @infinite_parameter(m, par in [0, 1])
    @infinite_parameter(m, par2 in [0, 1])
    @variable(m, 0 <= x <= 1, Infinite(par, par2), Bin)
    @variable(m, y == 1, Infinite(par), Int)
    d1 = @deriv(x, par)
    d2 = @deriv(y, par^2)
    d3 = @deriv(d1, par2)
    ...
    var = build_variable(error, d1, [0.5, NaN], check = false)
    rv = add_variable(m, var)
    data = TestData(par, 0, 1)
