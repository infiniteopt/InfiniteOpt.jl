...
IndependentParameter{UniDistributionSet{Normal{Float64}},FiniteDifference}(Normal{Float64}(μ=0.0, σ=1.0), DataStructures.SortedDict(-0.5=>Set([UserDefined]),0.5=>Set([UserDefined])), 12, FiniteDifference(Backward))
```
...
IndependentParameter{IntervalSet,FiniteDifference}([0, 10], DataStructures.SortedDict(0.0=>Set([UniformGrid]),3.33=>Set([UniformGrid]),6.67=>Set([UniformGrid]),10.0=>Set([UniformGrid])), 3, FiniteDifference(Backward))
```  
