...
function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, method::IP.InterpolationType; kwargs...)
    throw(ArgumentError("Unsupported interpolation type: $(method). Supported interpolation types are: linear_interpolation, constant_interpolation, and cubic_spline_interpolation."))
end

function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, interpMethod::typeof(IP.cubic_spline_interpolation); kwargs...)
...
function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, interpMethod::_irregularGridMethods; kwargs...)
    InfiniteOpt._check_result_is_current(JuMP.owner_model(vref), JuMP.value)
...
function JuMP.value(vref::InfiniteOpt.GeneralVariableRef,
    ...
    throw(ArgumentError("Unsupported interpolation degree: $(degree). Supported interpolation degrees are: Linear(), Constant(), and Cubic()."))
end

function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, degree::IP.Linear; kwargs...)
    return JuMP.value(vref, IP.linear_interpolation; kwargs...) 
end

function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, degree::IP.Constant; kwargs...)
    return JuMP.value(vref, IP.constant_interpolation; kwargs...) 
end

function JuMP.value(vref::InfiniteOpt.GeneralVariableRef, degree::IP.Cubic; kwargs...)
